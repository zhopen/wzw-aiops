...
if __name__ == '__main__':
    ...
    best_clusters = train_check(np.array(log_df.loc[:, ['len_zscore', 'ratio_zscore']]),args.maxclusters, isdraw=args.draw)
    print('最佳分类数为：' + str(best_clusters))
    
    print('正在分类数据...')
    log_df, kmeans = train(np.array(log_df.loc[:, ['len_zscore', 'ratio_zscore']]), best_clusters, log_df)
    print("完成！") 
